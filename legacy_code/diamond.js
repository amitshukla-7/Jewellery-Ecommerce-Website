...
function displayProducts(products) {
  ...
  products.forEach(product => {
    ...
    // ✅ Trust product.image if present, otherwise fallback
...
// 📥 Fetch Diamond Products Only
function fetchDiamondProducts() {
  ...
  onValue(dbRef, (snapshot) => {
    ...
    for (let key in data) {
      ...
      if (category === "diamond") {
        item.id = key; // ✅ Attach Firebase key as product ID
        diamondProducts.push(item);
      }
    }

    displayProducts(diamondProducts);
  });
}
...
if (searchInput) {
  searchInput.addEventListener("keydown", function (e) {
    if (e.key === "Enter") {
      ...
      const filtered = diamondProducts.filter(product =>
...
// 🚀 Load on Page Ready
window.addEventListener("DOMContentLoaded", fetchDiamondProducts);
