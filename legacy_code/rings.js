...
function displayProducts(products) {
  ...
  products.forEach(product => {
    ...
    // ✅ Use product.image directly (works with Firebase URLs, Base64, or filenames)
...
function fetchRingProducts() {
  ...
  onValue(dbRef, (snapshot) => {
    ...
    for (let key in data) {
      ...
      if (category === "rings") {
        item.id = key;
        ringProducts.push(item);
      }
    }

    displayProducts(ringProducts);
  });
}
...
if (searchInput) {
  searchInput.addEventListener("keydown", function (e) {
    if (e.key === "Enter") {
      ...
      const filtered = ringProducts.filter(product =>
...
// 🚀 On Load
window.addEventListener("DOMContentLoaded", fetchRingProducts);
